...
./qconcept -a $CONCEPT2 $PREVLINE 'регистрировать ?что событие (?на-что значение ?какой 1, ?для-чего поле).'; PREVLINE=`echo "$PREVLINE + 1" | bc`
./qconcept -a $CONCEPT2 $PREVLINE 'устанавливать ?что обработчик (?чего событие, ?какой \"выполнять ?что (=процедура ?что иметь ?что имя ?какой X=1).\").'; PREVLINE=`echo "$PREVLINE + 1" | bc`
./qconcept -a $CONCEPT2 $PREVLINE 'регистрировать ?что событие (?на-что значение ?какой 2, ?для-чего поле).'; PREVLINE=`echo "$PREVLINE + 1" | bc`
./qconcept -a $CONCEPT2 $PREVLINE 'устанавливать ?что обработчик (?чего событие, ?какой \"выполнять ?что (=процедура ?что иметь ?что имя ?какой X=2).\").'; PREVLINE=`echo "$PREVLINE + 1" | bc`
./qconcept -a $CONCEPT2 $PREVLINE 'регистрировать ?что событие (?на-что значение ?какой 3, ?для-чего поле).'; PREVLINE=`echo "$PREVLINE + 1" | bc`
./qconcept -a $CONCEPT2 $PREVLINE 'устанавливать ?что обработчик (?чего событие, ?какой \"выполнять ?что (=процедура ?что иметь ?что имя ?какой X=3).\").'; PREVLINE=`echo "$PREVLINE + 1" | bc`
./qconcept -a $CONCEPT2 $PREVLINE 'создавать ?что (=поле ?что иметь ?что имя ?какой Y).'; PREVLINE=`echo "$PREVLINE + 1" | bc`
./qconcept -a $CONCEPT2 $PREVLINE 'регистрировать ?что событие (?на-что значение ?какой 3, ?для-чего поле).'; PREVLINE=`echo "$PREVLINE + 1" | bc`
./qconcept -a $CONCEPT2 $PREVLINE 'устанавливать ?что обработчик (?чего событие, ?какой \"выполнять ?что (=процедура ?что иметь ?что имя ?какой Y=3).\").'; PREVLINE=`echo "$PREVLINE + 1" | bc`
